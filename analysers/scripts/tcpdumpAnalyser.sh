...
PS3="Select the option (press enter to show): "
...
do
	...
	echo -e "\n Enter the IP source:\n"
	read IP
	echo -e "\033[0;32m tcpdump -vnr $PCAP_FILE 'tcp[13] & 1 != 0' and src host $IP | grep -v 'tos' | sort -uV | cut -d " " -f5 | cut -d "." -f5 | uniq -c \033[0m \n"
	tcpdump -vnr $PCAP_FILE 'tcp[13] & 1 != 0' and src host $IP | grep -v "tos" | sort -uV | cut -d " " -f5 | cut -d "." -f5 | uniq -c
